...
artificial_tide = 'y'
time_generating_step = 1/10        # timesteps to generate the resultant output data. 
'''
------------------------ Key parameters to change as either y/n ----------------------------------
'''
...
def ebm(W_m, h, Q_r, Q_m, S_l, Q_l, S_oc, length, time_array):
    ...
    for C_k_iteration in range(1):
        ...
        # This makes C_k smalller, so when river is above the mean we reduce from the vel/ur component. 
        C_k[mask] = ((Ro_s[mask] / 1000)**20) * (((vel_tide[mask]) / ur[mask])**mid_comp[C_k_iteration]) * np.exp(end_comp[C_k_iteration] * Eta[mask]) # plotted everything from here agsint ck and Su and nothing lines up
        C_k[~mask] =  200*((Ro_s[~mask] / 1000)**20) * ((vel_tide[~mask]) / ur[~mask])**mid_comp[C_k_iteration] * np.exp(end_comp[C_k_iteration] * Eta[~mask])
        ...
        for i in range(len(Q_l)):          # Determine the flood or ebb, pos = flood, neg = ebb
            ...
            # print(S_u[i])
            # print("S_l:", S_l[i, mask_used])
            # print("Q_l:", Q_l[i, mask_used])
            # print("S_oc:", S_oc[i, mask_used])
            # print("k_x:", k_x[i, mask_used])
            # print("h:", h[:, mask_used].flatten())
            # print("W_m:", W_m[:, mask_used].flatten())
            # print("L_x:", Lx[i, mask_used])
            # print("Q_r:", Q_r[i, mask_used])
            # print("Q_l:", Q_l[i, mask_used])
                previous_S_u[mask_used] = S_u[i, mask_used]
            # print('yep')
            
            # S_u_no_kx[i, mask_used] =  h[:, mask_used].flatten() * W_m[:, mask_used].flatten() * (S_oc[i, mask_used] / (Lx[i, mask_used] * 1000)) / (Q_r[i, mask_used] + Q_l[i, mask_used])
            # stored_ck.append(k_x[i, mask_used])
            # print(k_x[i, mask_used])
