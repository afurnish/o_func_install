...
class DataChoice:
    ...
    def multi_select(self, ms, choice):
        if ms > 1:
            
            if choice == 'r':
                print("Choices reset.")
            else:
                print('Multiple selection, please make ', ms,' choices from the above.')

            
        
    def select(self, path, selection, ms, input_message = '' ): 
    ...
    def dir_select(self, start_path, location_of_choices, ms = 1):
        ...
        full_path = self.select(path,dirs,input_message, ms)
        return full_path
    ...
    def var_name_select(self,start_path, choices):
        a = 1
