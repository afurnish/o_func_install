...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  8 16:55:04 2025
...
# -*- coding: utf-8 -*-
"""
Storm-surge snapshot for unstructured UGRID:
- COMPUTE: find the single timestamp with the highest (spatial-metric) surface height.
- SAVE:    tiny "storm_surge_snapshot.nc" beside your plots (mesh + sh_at_peak + time_peak).
- PLOT:    render that snapshot; optional bathymetry-based dry overlay you can toggle.

Notes
- The "peak" is determined from a chosen spatial metric over faces (default: domain mean).
- You can restrict the metric to a coastal bounding box if desired.
"""
...
# Path to scattered bathymetry (lon, lat, z). Recomputed each run if overlay is ON.
...
# Choose how to detect the surge timestamp from the time series of surface height:
#   "mean"  -> domain mean over selected faces
#   "p90"   -> 90th percentile over selected faces (robust to local outliers)
SURGE_METRIC = "mean"

# Optional coastal selection for the metric (faces inside this box drive the peak time).
# Set COAST_BOX = None to use the full domain.
# COAST_BOX = (xmin, xmax, ymin, ymax)
COAST_BOX = None
# Example for your domain:
# COAST_BOX = (-3.6, -2.75, 53.2, 54.5)

# Plot extents; None -> auto from mesh (you can pass one-sided limits too)
X_LIMITS = (-3.6, -2.75)
Y_LIMITS = None

# Color limits for surface height plot
SSH_CLIM = (0, 10)
SSH_CMAP = cmo.cm.dense

# Dry overlay switch + parameters
PLOT_BED_OVERLAY = True         # toggle ON/OFF
DRY_THRESH_M     = 0.02         # mark dry if (sh_at_peak - bed) <= DRY_THRESH_M
DRY_COLOR        = "#8B5A2B"    # brown

# I/O engine + variable name candidates
ENGINE        = "netcdf4"
...
def compute_storm_surge_snapshot(
    ...
    save a compact file with the mesh + sh_at_peak + time_peak.
    """
    ...
    # locate surface height variable
    ...
    # Optional coastal box to drive the metric
    if coast_box is not None:
        xmin, xmax, ymin, ymax = coast_box
        in_box = (cx >= xmin) & (cx <= xmax) & (cy >= ymin) & (cy <= ymax)
        if not np.any(in_box):
            print("[STORM] Warning: COAST_BOX selected zero faces; using full domain.")
            in_box = np.ones_like(cx, dtype=bool)
    else:
        in_box = np.ones_like(cx, dtype=bool)

    # Build the spatial metric time series
    ...
    # snapshot field (1-D over faces) at that time
    sh_at_peak = sh.isel(time=t_idx)
    sh_at_peak = sh_at_peak.squeeze()
    # Ensure it's 1-D over faces
    for d in list(sh_at_peak.dims):
        if d != "mesh2d_nFaces":
            sh_at_peak = sh_at_peak.mean(d)

    # Save a compact file with mesh + snapshot + the time
    snap = xr.Dataset(
        data_vars=dict(
...
# --------- Bathy interpolation (recomputed every run) ----------
...
def _plot_snapshot_with_optional_bed(
    ...
    if plot_bed_overlay:
        ...
        depth = vals - bed  # positive = water above bed
...
def plot_storm_snapshot(snapshot_nc: Path, out_dir: Path,
    ...
    title = f"Storm-surge snapshot (peak metric) — {t_peak:%Y-%m-%d %H:%M UTC}"
...
if __name__ == "__main__":
    # Define your runs (dataset -> output folder)
    runs = [
    ...
    for DS_PATH, OUT_DIR in runs:
        ...
        # 2) plot it (toggle bed overlay with PLOT_BED_OVERLAY)
        plot_storm_snapshot(
