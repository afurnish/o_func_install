...
Additions:
- Interpolate scattered bathy (lon,lat,z) from XYZ onto face centroids (recomputed every run)
- Make separate dry masks for sh_min_mean and sh_max_mean (threshold DRY_THRESH_M)
- Plot surface height with linear cmo.dense for wet faces + brown overlay for dry faces
- Leave salinity plots unchanged (no dry overlay)
"""
...
# ======= DRY MASK & PLOT TUNING =======
DRY_THRESH_M  = -1     # dry if depth <= 2 cm
DRY_COLOR     = "#8B5A2B" # brown overlay for dry faces (min & max)
# ======================================

# Colormaps (as in your script)
...
# Plot options
HEIGHT_CLIM      = (-4, 4)     # set None to use recommended symmetric auto (printed)
...
def _repair_connectivity_da(da, n_nodes=None, label="connectivity"):
    ...
    if (a < 0).any() and not np.all(a[a < 0] == -1):
        ...
        raise ValueError(f"{label} (repair): unexpected negatives after repair: {bad}")
...
def _sanitize_all_topologies(base: xr.Dataset, donor_path: str | None = None) -> xr.Dataset:
    ...
    for cand in ("mesh2d_node_x", "mesh2d_node_y"):
        if cand in base:
            n_nodes = int(base[cand].shape[0]); break
...
def _compute_face_centroids(face_nodes, node_x, node_y):
    ...
    for i in range(nfaces):
        idxs = face_nodes[i]; idxs = idxs[idxs >= 0]
        if idxs.size >= 3:
...
def _poly_from_faces(face_nodes, node_x, node_y, mask=None, stride=1):
    ...
    for i in range(0, nfaces, max(1, stride)):
        if mask is not None and not mask[i]:
            continue
        idxs_i = face_nodes[i]
        idxs_i = idxs_i[idxs_i >= 0]
        if idxs_i.size < 3:
            continue
        xs = node_x[idxs_i]; ys = node_y[idxs_i]
...
def _poly_plot(face_nodes, node_x, node_y, values_1d, out_png, clim, title,
    ...
    """Signature matches your original: (face_nodes, node_x, node_y, values_1d, ...)"""
    polys, good = _poly_from_faces(face_nodes, node_x, node_y, mask=None, stride=stride)

    fig, ax = plt.subplots(figsize=(5.5, 7))
    ax.set_facecolor("lightgrey")

    arr = np.asarray(values_1d).reshape(-1)   # ensure rank-1 for matplotlib
    coll = PolyCollection(polys, array=arr[good], cmap=cmap)
...
def _plot_surface_with_dry_overlay(values_1d, dry_mask, face_nodes, node_x, node_y,
                                   out_png, clim, title, stride=1, cmap=None,
                                   xlim=None, ylim=None, cbar_label="Surface Height [m]"):
    """Two-layer plot: wet faces with colormap, dry faces solid brown overlay."""
    fig, ax = plt.subplots(figsize=(5.5, 7))
    ax.set_facecolor("lightgrey")

    values_1d = np.asarray(values_1d).reshape(-1)
    dry_mask = np.asarray(dry_mask, dtype=bool)
    wet_mask = ~dry_mask

    # Wet layer
    wet_polys, wet_idx = _poly_from_faces(face_nodes, node_x, node_y, mask=wet_mask, stride=stride)
    if wet_idx.size:
        coll = PolyCollection(wet_polys, array=values_1d[wet_idx], cmap=cmap)
        coll.set_rasterized(True)
        if clim is not None:
            coll.set_clim(*clim)
        ax.add_collection(coll)
    ...
    # Dry overlay (solid brown)
    if dry_mask.any():
        dry_polys, _ = _poly_from_faces(face_nodes, node_x, node_y, mask=dry_mask, stride=stride)
        if len(dry_polys):
...
def _interpolate_bathy_to_faces(xyz_path: Path, node_x, node_y, face_nodes):
    """Read lon,lat,z XYZ and interpolate z to face centroids (linear)."""
...
def plot_results(
    ...
    # --- Interpolate bathymetry (recompute every run) ---
    print(f"[BATHY] Interpolating XYZ -> face centroids from {BED_XYZ_PATH}")
    bed_face = _interpolate_bathy_to_faces(BED_XYZ_PATH, node_x, node_y, face_nodes)
    # Depths relative to bed
    depth_min = ds["sh_min_mean"].values - bed_face
    depth_max = ds["sh_max_mean"].values - bed_face
    # Separate dry masks
    dry_mask_min = np.isfinite(depth_min) & (depth_min <= DRY_THRESH_M)
    dry_mask_max = np.isfinite(depth_max) & (depth_max <= DRY_THRESH_M)
    print(f"[DRY] min dry faces: {int(dry_mask_min.sum())} | max dry faces: {int(dry_mask_max.sum())}")

    # --- Plot surface height with dry overlays ---
    _plot_surface_with_dry_overlay(
        ds["sh_min_mean"].values, dry_mask_min, face_nodes, node_x, node_y,
        out_dir / "unstruct_surface_height_rollingMinMean_3h.png",
        clim=height_clim, title="Surface height: mean rolling 3h MIN",
        stride=poly_stride, cmap=cmap_topo, xlim=x_limits, ylim=y_limits,
        cbar_label="Surface Height [m]"
    )
    _plot_surface_with_dry_overlay(
        ds["sh_max_mean"].values, dry_mask_max, face_nodes, node_x, node_y,
    ...
    # --- Salinity plots unchanged (no dry overlay) ---
