...
class Stats:
    ...
    def linear_regression(self, figpath, data_stats_path):
        ...
        for variable_name in self.common_keys: # set up common keys to remove bathymetry etc. 
            ...
            def timeseries_plot(list_of_data):
                """
                UPDATED (plotting-only changes + shared best-14-day window):
            
                Requirements you asked for:
                - Pick the *best RMSE* 14-day period (objective), BUT
                - Use the SAME 14-day window for BOTH Liverpool + Heysham
                - Keep your original data access logic: mk = first key in model[i], y = model[i][mk]
                - No titles/stats added to the figure (legend only)
                - Clear line hierarchy + draw order + clean ticks
            
                How "best" is chosen:
                - Minimises the MEAN RMSE of (IRENE - Tide Gauge) across ALL tide gauges in self.tide_save
                  (i.e. one window that is jointly best for the set).
                """
            
                #%% ------------------------------------------------------------
                # Helper functions (local so you can drop this straight in)
                # ------------------------------------------------------------
                def _as_np(x):
                    return x.to_numpy() if hasattr(x, "to_numpy") else np.asarray(x)
            
                def _infer_dt_hours(tt):
                    tt = np.asarray(tt)
                    if len(tt) < 2:
                        return 1.0
                    return float((tt[1] - tt[0]) / np.timedelta64(1, "h"))
                ...
                def _get_series_for_one_gauge(model_like, gauge_i):
                    """Your original indexing: mk = first key in model[gauge_i]; return series."""
                    mk = [j for j in model_like[gauge_i].keys()][0]
                    return model_like[gauge_i][mk]
            
                #% ------------------------------------------------------------
                # Decide which object in list_of_data is Tide Gauge and which is IRENE
                # (using the same naming logic you already use)
                # ------------------------------------------------------------
                if variable_name != 'surface_height':
                    # (kept for completeness; your original timeseries plot usually relevant for surface_height)
                    data_iter = list_of_data[1:]
                    mod_key_new = [r'UKC4$_{\mathrm{PRIMEA}}$', r'UKC4$_{\mathrm{ao}}$']
                else:
                    data_iter = list_of_data
                    mod_key_new = ['Tide Gauge', r'UKC4$_{\mathrm{PRIMEA}}$', r'UKC4$_{\mathrm{ao}}$']
            
                # Map names -> the corresponding object in data_iter
                name_to_obj = {}
                for kil, obj in enumerate(data_iter):
                    if mod_key_new[kil] == r'UKC4$_{\mathrm{PRIMEA}}$':
                        nm = 'IRENE'
                    elif mod_key_new[kil] == r'UKC4$_{\mathrm{ao}}$':
                        nm = 'UKC4'
                ...
                if "Tide Gauge" not in name_to_obj or "IRENE" not in name_to_obj:
                    raise ValueError("Expected both 'Tide Gauge' and 'IRENE' to be present for shared best-window selection.")
            
                #% ------------------------------------------------------------
                # Find ONE shared best 14-day window based on mean RMSE(IRENE - Tide Gauge)
                # across ALL gauges (Liverpool + Heysham etc.)
                # ------------------------------------------------------------
                tt = np.asarray(self.time_sliced)
                dt_h = _infer_dt_hours(tt)
                window_days = 12
                win_len = int(round(window_days * 24.0 / dt_h))
                win_len = max(win_len, 10)
            
                n = len(tt)
                if n <= win_len + 1:
                    best_s0, best_s1 = 0, n - 1
                else:
                    best_s0 = 0
                    best_score = np.inf
            
                    # Pre-load obs and irene arrays per gauge to make scoring cheap
                    obs_by_g = []
                    irn_by_g = []
                    for gi, _ in enumerate(self.tide_save):
                        obs = _as_np(_get_series_for_one_gauge(name_to_obj["Tide Gauge"], gi)).astype(float)
                        irn = _as_np(_get_series_for_one_gauge(name_to_obj["IRENE"], gi)).astype(float)
                        obs_by_g.append(obs)
                        irn_by_g.append(irn)
            
                    # Slide window, compute mean RMSE across gauges
                    for s0 in range(0, n - win_len):
                        s1 = s0 + win_len
                        rmses = []
                        good = True
            
                        for gi in range(len(obs_by_g)):
                            oseg = obs_by_g[gi][s0:s1]
                            mseg = irn_by_g[gi][s0:s1]
                            r = _rmse(mseg, oseg)
                    ...
                    best_s1 = min(n - 1, best_s0 + win_len)
            
                #% ------------------------------------------------------------
                # Plot each gauge using the SAME best_s0/best_s1
                # ------------------------------------------------------------
                STYLE = {
                ...
                for i, tide_gauge in enumerate(self.tide_save):
                    ...
                    # Plot in fixed order for clarity (gauge behind)
                    for nm in ["Tide Gauge", "IRENE", "UKC4"]:
                        if nm not in name_to_obj:
                            continue
            
                        series_i = _get_series_for_one_gauge(name_to_obj[nm], i)
                        st = STYLE.get(nm, dict(color="0.4", lw=1.2, ls="--", alpha=0.9, zorder=3))
                        ...
                        # keep your scatter marker logic (unchanged)
                        high_tide_num = 174 + 13 + 12
                        ax.scatter(
                            pd.Timestamp(tt[high_tide_num].item()).to_pydatetime(),
                            series_i.iloc[high_tide_num] if hasattr(series_i, 'iloc') else series_i[high_tide_num],
                            s=4
                        )
            
                    # Shared best 14-day window for ALL gauges
                    ax.set_xlim([tt[best_s0], tt[best_s1]])
            
                    tide_gauge_name = [j for j in self.tide_loc_dict.keys()][i]
            
                    # Legend only (no titles)
                    ax.legend(loc="upper center", bbox_to_anchor=(0.5, 1.18), ncol=3, frameon=False)
            
                    # Cleaner ticks for ~14 days
                    ax.xaxis.set_major_locator(mdates.DayLocator(interval=2))
                    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
                    fig.autofmt_xdate()
            
                    # y-label logic (unchanged)
                    if variable_name == 'surface_height':
                        variable_name_normal = 'Surface Height'
                    elif variable_name == 'salinity':
                        variable_name_normal = 'Salinity'
                    ...
                    plt.tight_layout()
                    #%%
                    fig.savefig(os.path.join(fig_path, f"timeseries_{variable_name}_{tide_gauge_name}_best14d.png"), dpi=300)
                    plt.close(fig)
